...
export default class Server {
  ...
  async generateHandlersFromFiles(): Promise<Handler[]> {
    ...
    for (const file of configurationFileContent.classPaths) {
      const { path, className, functionNames } = await bundleJavascriptCode(
        file
      );
  ...
  async start(handlers: Handler[]) {
    if (handlers.length === 0) {
      console.log(
        "No class registered. Make sure that you have set the classes that you want to deploy in the genezio.yaml configuration file."
      );
      return;
    }
    ...
    const requestListener = async function (
      ...
      request.on("end", async function () {
        ...
        const handler = handlers.find(
          (handler) =>
            handler.className === className && handler.functionName === method
        );
        ...
        try {
          const functionResponse = await handler.object[functionName](
