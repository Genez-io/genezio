...
class SdkGenerator implements SdkGeneratorInterface {
        ...
        for (const classInfo of sdkGeneratorInput.classesInfo) {
            ...
            for (const methodDefinition of classDefinition.methods) {
                ...
                methodView.parameters = methodDefinition.params.map((e) => {
                    return {
                        name:
                            (PYTHON_RESERVED_WORDS.includes(e.name) ? e.name + "_" : e.name) +
                            ": " +
    ...
    generateExternalType(type: Node): string {
        if (type.type === AstNodeType.TypeAlias) {
            ...
            typeAlias.typeLiteral.properties.sort((a: PropertyDefinition, b: PropertyDefinition) =>
                ...
                `class ${typeAlias.name}:\n\tdef __init__(self, ${typeAlias.typeLiteral.properties
                    .map(
                        (e: PropertyDefinition) =>
                            `${e.name}: ${this.getParamType(e.type)}${e.optional ? " = None" : ""}`,
                    )
                    .join(", ")}):\n\t\t` + // header
                `${typeAlias.typeLiteral.properties
                    .map(
                        (e: PropertyDefinition) =>
                            `${e.optional ? `if ${e.name} is not None:\n\t\t\t` : ""}${
                                e.type.type === AstNodeType.CustomNodeLiteral
                                    ? `if isinstance(${e.name}, Mapping):\n\t\t\t${
                                          e.optional ? "\t" : ""
                                      }self.${e.name} = ${this.getParamType(e.type)}(**${
                                          e.name
                                      })\n\t\t${e.optional ? "\t" : ""}else:\n\t\t\t${
                                          e.optional ? "\t" : ""
                                      }self.${e.name} = ${e.name}`
    ...
    addImportToCurrentView(currentView: ModelView, externalType: Node) {
        ...
        for (const importType of currentView.imports) {
            if (
                importType.path === externalType.path?.replace("/", ".") &&
