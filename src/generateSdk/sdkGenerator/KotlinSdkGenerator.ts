...
import { castArrayRecursivelyInitial, castMapRecursivelyInitial, getParamType } from "../../utils/kotlinAstCasting.js";
              ...
              if (elem.type === AstNodeType.ClassDefinition) {
                  ...
                  for (const methodDefinition of classDefinition.methods) {
                      ...
                      methodView.parameters = methodDefinition.params.map((e) => {
                          return {
                              name: (KOTLIN_RESERVED_KEYWORDS.includes(e.name) ? e.name + "_" : e.name) + ": " + getParamType(e.paramType),
                              last: false
                          }
                      });
                      ...
                      methodView.sendParameters = methodDefinition.params.map((e) => {
                          return {
                              name: (KOTLIN_RESERVED_KEYWORDS.includes(e.name) ? e.name + "_" : e.name),
                              last: false
                          }
                      });
