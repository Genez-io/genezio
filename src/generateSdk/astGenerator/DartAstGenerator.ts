...
import {
    checkIfDartIsInstalled,
    getDartAstGeneratorPath,
    getDartSdkVersion,
} from "../../utils/dart.js";
...
export class AstGenerator implements AstGeneratorInterface {
    ...
    async #compileGenezioDartAstExtractor(genezioAstExtractorPath: string) {
        ...
        await runNewProcess(
            `dart compile aot-snapshot main.dart -o ${genezioAstExtractorPath}`,
            folder,
        );
    ...
    async generateAst(input: AstGeneratorInput): Promise<AstGeneratorOutput> {
        ...
        const { directory: genezioAstExtractorDir, path: genezioAstExtractorPath } =
            getDartAstGeneratorPath(dartSdkVersion);
