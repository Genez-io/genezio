...
export class NodeTsBundler implements BundlerInterface {
    ...
    async #bundleTypescriptCode(filePath: string, tempFolderPath: string): Promise<void> {
    ...
    async bundle(input: BundlerInput): Promise<BundlerOutput> {
        ...
        await this.#bundleTypescriptCode(input.configuration.path, temporaryFolder)

        // 2. Copy non js files
        await this.#copyNonTsFiles(temporaryFolder)
