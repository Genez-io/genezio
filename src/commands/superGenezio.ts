...
import { deleteFolder, fileExists, getAllFilesFromCurrentPath, readUTF8File, writeToFile } from "../utils/file.js";
...
import { BACKEND_TEMPLATE_GIT_URL, FULLSTACK_TEMPLATE_GIT_URL } from "../generateSdk/templates/newProject.js";
...
export async function genezioCommand() {
  ...
  const templateAnswer: Answers = await inquirer.prompt([
    {
      ...
      choices: (Object.keys(GenezioCommandTemplates) as (keyof typeof GenezioCommandTemplates)[]).map(
        (key) => {
  ...
  log.info(colors.cyan(`Your project will be named ${colors.green(projectNameAnswer.projectName)}.\n`));
  ...
  log.info(colors.cyan(`Your project will be deployed in ${colors.green(regionAnswer.region)}.\n`));
  ...
  if (!emptyDirectory) {
    ...
    log.info(colors.cyan(`We are creating the project in ${colors.green(`./${confirmAnswer.directoryName}`)}.\n`));
...
export async function prepareProjectFolder(
  ...
  const cloneRes = await runNewProcessWithResultAndReturnCode(`git clone ${gitUrl} ${directoryName}`);
  ...
  const genezioYamlContent = (await readUTF8File(path.join(process.cwd(), directoryName, "genezio.yaml")))
  ...
  if (genezioYamlContent.includes("frontend: client")) {
    ...
    const clientPackageJsonContent = (await readUTF8File(path.join(process.cwd(), directoryName, "client", "package.json")))
      .replaceAll("@genezio-sdk/template-name_us-east-1", `@genezio-sdk/${projectName}_${region}`);

    await writeToFile(path.join(process.cwd(), directoryName, "client"), "package.json", clientPackageJsonContent);

    // replace in src/App.tsx
    const clientAppTsxContent = (await readUTF8File(path.join(process.cwd(), directoryName, "client", "src", "App.tsx")))
      .replaceAll("@genezio-sdk/template-name_us-east-1", `@genezio-sdk/${projectName}_${region}`);

    await writeToFile(path.join(process.cwd(), directoryName, "client", "src"), "App.tsx", clientAppTsxContent);
